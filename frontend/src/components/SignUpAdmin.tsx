...
function SignUpAdmin() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const apiUrlrequest= 'http://localhost:8080/request-admin';
            ...
            if (response.ok) {
                const responseRegister = await fetch(apiUrlrequest, requestOptionsRequest);
                const dataRegister = await responseRegister.json();
                if (responseRegister.ok) {
                    await Swal.fire({
                        title: 'ส่งคำขอเรียบร้อยแล้ว',
                        text: 'กรุณารอการอนุมัติจากผู้ดูแลระบบ และคุณจะได้รับการแจ้งเตือนผ่านทางอีเมลเมื่อการอนุมัติเสร็จสมบูรณ์',
                        icon: 'success',
                        confirmButtonText: 'รับทราบ'
                    });
    ...
    return (
        <div className='signup-admin-page'>
            ...
            <div className="signup-admin-container">
                ...
                <button type="submit" onClick={generateOTP} className='signup-admin-button'>Sign Up</button>
            </div> 
        </div>
    );
};
